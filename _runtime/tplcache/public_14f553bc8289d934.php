...
<div class="person-info clearfix">
    <dl class="person-info-t clearfix">
        <dd>
            ...
            <?php if($mid==$uid || CheckPermission('core_normal','read_data')){ ?>
    ...
    more_operation: {
        ...
        // }
                ...
                <!--<div class="app-tab-menu clearfix">
                    ...
                    <dl>
                        <dt><a href="<?php echo U('public/Profile/index',array('type'=>$type,'feed_type'=>'','uid'=>$uid));?>" class="current">微博</a><a href="<?php echo U('public/Profile/data',array('uid'=>$uid));?>">资料</a></dt>
                    </dl>
                </div>-->
        </div>
                ...
                <div class="extend minh">
                    ...
                    <div class="feed_lists">
                        ...
                        <dl class="feed_list feed_list_hover detail">
                            <dd class="content" id="feed<?php echo ($feedInfo["feed_id"]); ?>">
                                
                                <p class="clearfix">
                                    <span event-node="show_admin" event-args="feed_id=<?php echo ($feedInfo['feed_id']); ?>&uid=<?php echo ($feedInfo['uid']); ?>&feed_del=<?php echo CheckPermission('core_admin','feed_del');?>&channel_recommend=<?php echo CheckPermission('channel_admin','channel_recommend');?>&isrefresh=1" href="javascript:;" class="right f12 hover f9" style="display:none;cursor:pointer">管理</span>
                                <?php echo (format($feedInfo["body"],true)); ?>
                                </p>
                                <p class="info">
                                <span class="right">
                                    <?php $cancomment_old = empty($feedInfo['app_row_id'])  ? 0 : 1; ?>
                                    <?php $cancomment = intval(CheckPermission('core_normal','feed_comment')); ?>
                                    <?php if(($feedInfo["actions"]["repost"])  ==  "true"): ?><?php if(CheckPermission('core_normal','feed_share') && in_array('repost',$weibo_premission)){ ?>
                                              <?php $sid = !empty($feedInfo['app_row_id'])? $feedInfo['app_row_id'] : $feedInfo['feed_id']; ?>
                                                <?php echo W('Share',array('sid'=>$sid,'stable'=>$feedInfo['app_row_table'],'initHTML'=>'','current_table'=>'feed','current_id'=>$feedInfo['feed_id'],'nums'=>$feedInfo['repost_count'],'appname'=>$feedInfo['app'],'cancomment'=>$cancomment,'feed_type'=>$feedInfo['type'],'is_repost'=>$feedInfo['is_repost']));?>
                                                <i class="vline">|</i>
                                        <?php } ?><?php endif; ?>
                                    <?php if(($feedInfo["actions"]["favor"])  ==  "true"): ?><?php echo W('Collection',array('type'=>'btn','sid'=>$feedInfo['feed_id'],'stable'=>'feed','sapp'=>$feedInfo['app']));?><?php endif; ?>
                                    <?php if(in_array('comment',$weibo_premission)): ?>
                                    <i class="vline">|</i>  
                                    <a href="javascript:void(0)"><?php echo L('PUBLIC_STREAM_COMMENT');?>
                                    <?php if(($feedInfo["comment_count"])  !=  "0"): ?>(<?php echo ($feedInfo['comment_count']); ?>)<?php endif; ?></a>
                                    <?php endif; ?>
                                </span>
                            <a class="date" href="javascript:;"><?php echo (friendlyDate($feedInfo["publish_time"])); ?></a>
                            <span><?php echo ($feedInfo['from']); ?></span>
                            <em class="hover">
                            <?php if($feedInfo['uid'] != $GLOBALS['ts']['mid']){ ?>
                            <?php if(CheckPermission('core_normal','feed_report')){ ?>
                            <a href="javascript:void(0)" event-node='denounce' event-args='aid=<?php echo ($feedInfo["feed_id"]); ?>&type=feed&uid=<?php echo ($feedInfo["uid"]); ?>'><?php echo L('PUBLIC_STREAM_REPORT');?></a>
                            <?php } ?>
                            <?php } ?>
                            <i class="vline"></i>
                            <?php if(($feedInfo["actions"]["delete"])  ==  "true"): ?><!-- 做普通删除权限 和 管理删除权限 判断 & 只有微博可以被删除  -->
                            <?php if(($feedInfo['uid'] == $GLOBALS['ts']['mid']) ) { ?>
                            <a href="javascript:void(0)" event-node ='delFeed' event-args='feed_id=<?php echo ($feedInfo["feed_id"]); ?>&uid=<?php echo ($feedInfo["uid"]); ?>&isrefresh=1'><?php echo L('PUBLIC_STREAM_DELETE');?></a>
                            <?php } ?><?php endif; ?>
                            </em>
                              </p>
                              <p class="border">
						...
						<dl class="feed_list feed_comment">
							<dd class="content">
                                <div class="repeat clearfix pading">
                                    <?php $cancomment = CheckPermission('core_normal','feed_comment') && in_array('comment',$weibo_premission) ? 1: 0; ?>
                        <?php echo W('Comment',array('tpl'=>'detail','app_row_id'=>$feedInfo['app_row_id'],'app_uid'=>$feedInfo['uid'],'row_id'=>$feedInfo['feed_id'],'pageHtml'=>1,'limit'=>'20','order'=>'DESC','cancomment'=>$cancomment,'cancomment_old'=>0,'app_name'=>$feedInfo['app']));?>                                   
                                </div>
							</dd>
						</dl>
					</div>
