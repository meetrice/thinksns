...
<div id="page-wrap">
	<div id="main-wrap">
		<div class="item-announcement">
			<div class="hd"><?php echo L('PUBLIC_ANNOUNCEMENT');?></div>
			<div class="mod-announcement boxShadow">
				<h4><?php echo ($announcement["title"]); ?><span><?php echo (friendlyDate($announcement["mtime"],'full')); ?></span></h4>
				<?php echo ($announcement["content"]); ?>
				<div class="file-list">
					<ul>
						<?php if(!empty($announcement['attachInfo'])) { ?>
						<?php if(is_array($announcement["attachInfo"])): ?><?php $i = 0;?><?php $__LIST__ = $announcement["attachInfo"]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li><a href="<?php echo U('widget/Upload/down', array('attach_id'=>$vo['attach_id']));?>"><i class="ico-<?php echo ($vo["extension"]); ?>-small"></i><?php echo ($vo["name"]); ?></a><span>(<?php echo (byte_format($vo["size"])); ?>)</span></li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
						<?php } ?>
					</ul>
				</div>
			</div>
